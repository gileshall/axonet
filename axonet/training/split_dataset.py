"""
Split an unsplit dataset into train/val/test sets at the specimen (SWC) level.

Reads manifest.jsonl and creates separate manifest files for train/val/test
splits, ensuring all entries from a given specimen stay together in the same split.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Split an unsplit dataset into train/val/test sets at the specimen (SWC) level"
    )
    parser.add_argument(
        "--dataset-dir",
        type=Path,
        required=True,
        help="Directory containing the dataset with manifest.jsonl"
    )
    parser.add_argument(
        "--val-ratio",
        type=float,
        default=0.0,
        help="Fraction of specimens for validation set (default: 0.0)"
    )
    parser.add_argument(
        "--test-ratio",
        type=float,
        default=0.0,
        help="Fraction of specimens for test set (default: 0.0)"
    )
    parser.add_argument(
        "--split-seed",
        type=int,
    ...
    print(f"Found {len(unique_swc_files)} unique specimens (SWC files)")
    ...
    print(f"\nSpecimen split: train={len(train_swc)}, val={len(val_swc)}, test={len(test_swc)}")
    ...
    print(f"Entry counts: train={len(train_entries)}, val={len(val_entries)}, test={len(test_entries)}")
    ...
    print(f"  Train: {len(train_entries)} entries from {len(train_swc)} specimens")
    if val_swc:
        print(f"  Val:   {len(val_entries)} entries from {len(val_swc)} specimens")
    if test_swc:
        print(f"  Test:  {len(test_entries)} entries from {len(test_swc)} specimens")
    print(f"{'='*60}")
    print(f"\nOriginal manifest.jsonl is preserved.")
    if args.backup:
